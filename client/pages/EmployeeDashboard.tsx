...
export default function EmployeeDashboard() {
  ...
  useEffect(() => {
    getEmployeeTasks().then(tasks => {
      setAssignedTasks(tasks as any);
      setLoading(false);
    });
  }, [getEmployeeTasks]);
  ...
  return (
    <div className="min-h-screen bg-black relative pt-20">
      ...
      <div className="container max-w-7xl mx-auto px-4 py-8 relative z-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Team Member Dashboard – Home</h1>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {assignedTasks.filter(t => !t.isAccepted).map((task: any) => (
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <Card className="bg-black/80 border-white/10 backdrop-blur-xl border-dashed hover:border-primary/50 transition-all group relative overflow-hidden">
            ...
            <CardContent className="relative z-10">
              <p className="text-white/60 mb-6">Enter a room code provided by your Product Manager to join a team and start collaborating.</p>
          ...
          <Card className="bg-black/80 border-white/10 backdrop-blur-xl hover:border-accent/50 transition-all group relative overflow-hidden">
            ...
            <CardContent className="relative z-10">
              <p className="text-white/60 mb-6">Browse all the project rooms you've joined and access your assigned tasks.</p>
              <Button asChild variant="outline" className="w-full border-white/20 text-white hover:bg-white/10">
                <Link to="/rooms">Open Rooms <ArrowRight className="w-4 h-4 ml-2" /></Link>
              </Button>
            </CardContent>
          </Card>
        </div>
                    ...
                    <div key={task.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 hover:border-white/20 transition-all group">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="text-[10px] uppercase border-primary/20 text-primary">{task.roomName}</Badge>
                          <Badge variant="outline" className="text-[10px] uppercase border-white/10 text-white/40">{task.projectName}</Badge>
                        </div>
