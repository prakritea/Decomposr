...
export function RoomsProvider({ children }: { children: ReactNode }) {
  ...
  const refreshRooms = useCallback(async () => {
    ...
    try {
      const result = await api.rooms.getUserRooms(1, 50);
      setRooms(result.data);
      setPage(1);
      setTotalRooms(result.pagination.total);
      setHasMoreRooms(result.pagination.hasMore);
    } catch (error) {
  ...
  const loadMoreRooms = useCallback(async () => {
    try {
      ...
      const result = await api.rooms.getUserRooms(nextPage, 50);
  ...
  const createRoom = useCallback(async (name: string, description: string): Promise<ProjectRoom> => {
    try {
      ...
      await refreshRooms();
      toast({ title: "Room created!", description: `${name} is ready for collaboration` });
      return room;
    } catch (error) {
      toast({
  ...
  const joinRoom = useCallback(async (inviteCode: string): Promise<ProjectRoom> => {
    try {
      ...
      await refreshRooms();
      toast({ title: "Joined room!", description: `You are now a member of ${room.name}` });
      return room;
    } catch (error) {
      toast({
  ...
  const getEmployeeTasks = useCallback(async (): Promise<Task[]> => {
    try {
      return await api.projects.getMyTasks();
    } catch (error) {
